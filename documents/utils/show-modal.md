---
title: showModal - 命令式弹窗
order: 5
---

# showModal 命令式弹窗

命令式弹窗工具，支持灵活内容渲染、可拖拽、Promise 异步操作。告别繁琐的 `visible` 状态管理，一行代码打开弹窗。
...
| open    | 重新打开已关闭弹窗（极少用） | () => void                             |

## 在线演示

<code src="./demos/showModal-basic.tsx"></code>
...
const controller = showModal(renderStep(0), {
  ...
  onOk: () => {
    if (step < 2) {
      step++
      controller.update(renderStep(step), {
        title: `步骤 ${step + 1}`,
        okText: step === 2 ? '完成' : '下一步',
      })
      return false // 阻止关闭
    }
    return true
