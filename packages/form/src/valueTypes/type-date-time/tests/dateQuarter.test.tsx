...
describe('type="dateQuarter" 编辑模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="dateQuarter" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const { container } = render(<ProField className="test-picker" initialValue={value} type="dateQuarter" />)
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(
      run(dayjs(value), 'format', 'YYYY-[Q]Q'),
    )
  })
  ...
  it('can select date correctly', async () => {
    const value = Date.now()
    const valueFormat = run(dayjs(value), 'format', 'YYYY-[Q]Q')
    const { container } = render(<ProField className="test-picker" type="dateQuarter" props={{ open: true }} />)
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="dateQuarter" />)
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="dateQuarter" />
      </ConfigProvider>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const { container } = render(
      <ProField className="test-picker" initialValue={value} type="dateQuarter" format="YYYY~[Q]Q" />,
    )
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(valueFormat)
  })
})

describe('type="dateQuarter" 浏览模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="dateQuarter" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="dateQuarter" />
      </div>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="dateQuarter" format="YYYY~[Q]Q" />
      </div>,
    )
    await delay(100)
    expect(node.getByText(valueFormat)).toBeInTheDocument()
  ...
  it('tooltip will show when mouse enter', async () => {
    ...
    const valueFormat = run(valueDayjs, 'format', 'YYYY-[Q]Q')
    const valueFromNowFormat = run(valueDayjs, 'fromNow')
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={valueDayjs.valueOf()} type="dateQuarter" />
      </div>,
    )
    await delay(100)
