...
describe('type="dateWeek" 编辑模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="dateWeek" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const { container } = render(<ProField className="test-picker" initialValue={value} type="dateWeek" />)
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(
      run(dayjs(value), 'format', 'YYYY-wo'),
    )
  })
  ...
  it('can select date correctly', async () => {
    const value = Date.now()
    const valueFormat = run(dayjs(value), 'format', 'YYYY-wo')
    const selectorFormat = run(dayjs(value), 'format', 'YYYY-MM-DD')
    const { container } = render(<ProField className="test-picker" type="dateWeek" props={{ open: true }} />)
    fireEvent.click(document.querySelector(`.ant-picker-cell[title="${selectorFormat}"]`)!)
    await delay(300)
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="dateWeek" />)
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="dateWeek" />
      </ConfigProvider>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const { container } = render(
      <ProField className="test-picker" initialValue={value} type="dateWeek" format="YYYY~wo" />,
    )
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(valueFormat)
  })
})

describe('type="dateWeek" 浏览模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="dateWeek" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="dateWeek" />
      </div>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="dateWeek" format="YYYY~wo" />
      </div>,
    )
    await delay(100)
    expect(node.getByText(valueFormat)).toBeInTheDocument()
  ...
  it('tooltip will show when mouse enter', async () => {
    ...
    const valueFormat = run(valueDayjs, 'format', 'YYYY-wo')
    const valueFromNowFormat = run(valueDayjs, 'fromNow')
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={valueDayjs.valueOf()} type="dateWeek" />
      </div>,
    )
    await delay(100)
