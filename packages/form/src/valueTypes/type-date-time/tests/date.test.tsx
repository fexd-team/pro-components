...
describe('type="date" 编辑模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="date" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const { container } = render(<ProField className="test-picker" initialValue={value} type="date" />)
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(
      run(dayjs(value), 'format', 'YYYY-MM-DD'),
    )
  })
  ...
  it('can select date correctly', async () => {
    const value = Date.now()
    const { container } = render(<ProField className="test-picker" type="date" props={{ open: true }} />)
    const valueFormat = run(dayjs(value), 'format', 'YYYY-MM-DD')
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="date" />)
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="date" />
      </ConfigProvider>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const { container } = render(
      <ProField className="test-picker" initialValue={value} type="date" format="YYYY~MM~DD" />,
    )
    expect(container.querySelector('.test-picker .ant-picker-input > input')!.getAttribute('value')).toBe(valueFormat)
  })
})

describe('type="date" 浏览模式', () => {
  // 能够正确渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="date" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="date" />
      </div>,
    )
    await delay(100)
  ...
  it('format can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={value} type="date" format="YYYY~MM~DD" />
      </div>,
    )
    await delay(100)
    expect(node.getByText(valueFormat)).toBeInTheDocument()
  ...
  it('tooltip will show when mouse enter', async () => {
    ...
    const valueFormat = run(valueDayjs, 'format', 'YYYY-MM-DD')
    const valueFromNowFormat = run(valueDayjs, 'fromNow')
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={valueDayjs.valueOf()} type="date" />
      </div>,
    )
    await delay(100)
