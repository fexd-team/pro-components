...
describe('type="dateMonthRange" 编辑模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="dateMonthRange" />)
  ...
  it('can select date correctly', async () => {
    const startDayjsValue = dayjs()
    const endDayjsValue = dayjs().add(1, 'month')
    const { container } = render(<ProField className="test-picker" type="dateMonthRange" props={{ open: true }} />)
    const startDayjsValueFormat = startDayjsValue.format('YYYY-MM')
    fireEvent.click(document.querySelector(`.ant-picker-cell[title="${startDayjsValueFormat}"]`)!)
    const endDayjsValueFormat = endDayjsValue.format('YYYY-MM')
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="dateMonthRange" />)
    expect(
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="dateMonthRange" />
      </ConfigProvider>,
    )
    await delay(100)
    expect(
...
describe('type="dateMonthRange" 浏览模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="dateMonthRange" />)
