...
describe('type="dateRange" 编辑模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="dateRange" />)
  ...
  it('can select date correctly', async () => {
    const startDayjsValue = dayjs()
    const endDayjsValue = dayjs().add(1, 'month')
    const { container } = render(<ProField className="test-picker" type="dateRange" props={{ open: true }} />)
    const startDayjsValueFormat = startDayjsValue.format('YYYY-MM-DD')
    fireEvent.click(document.querySelector(`.ant-picker-cell[title="${startDayjsValueFormat}"]`)!)
    const endDayjsValueFormat = endDayjsValue.format('YYYY-MM-DD')
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="dateRange" />)
    expect(
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="dateRange" />
      </ConfigProvider>,
    )
    await delay(100)
    expect(
...
describe('type="dateRange" 浏览模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="dateRange" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const node = render(
      <div className="test-picker">
        <ProField mode="view" initialValue={[startDayjsValue.valueOf(), endDayjsValue.valueOf()]} type="dateRange" />
      </div>,
    )
    await delay(100)
