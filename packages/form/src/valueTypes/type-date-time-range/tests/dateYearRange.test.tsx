...
describe('type="dateYearRange" 编辑模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const { container } = render(<ProField className="test-picker" type="dateYearRange" />)
  ...
  it('initialValue can be set correctly', async () => {
    ...
    const endDayjsValue = dayjs().add(2, 'year')
  ...
  it('can select date correctly', async () => {
    const startDayjsValue = dayjs()
    const endDayjsValue = dayjs().add(2, 'year')
    const { container } = render(<ProField className="test-picker" type="dateYearRange" props={{ open: true }} />)
    const startDayjsValueFormat = startDayjsValue.format('YYYY')
    fireEvent.click(document.querySelector(`.ant-picker-cell[title="${startDayjsValueFormat}"]`)!)
    const endDayjsValueFormat = endDayjsValue.format('YYYY')
  ...
  it('can set locale correctly', async () => {
    const { container: container1 } = render(<ProField className="test-picker1" type="dateYearRange" />)
    expect(
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-picker2" type="dateYearRange" />
      </ConfigProvider>,
    )
    await delay(100)
    expect(
...
describe('type="dateYearRange" 浏览模式', () => {
  // date 类型能够正确的渲染
  it('date type can render correctly', async () => {
    const node = render(<ProField mode="view" type="dateYearRange" />)
  ...
  it('tooltip will be affected by localeKey', async () => {
    ...
    const endDayjsValue = dayjs().locale('zh-cn').add(2, 'year')
    const startDayjsValueFormat = startDayjsValue.format('YYYY')
    const endDayjsValueFormat = endDayjsValue.format('YYYY')
    const startDayjsValueFormatFromNow = startDayjsValue.fromNow()
    const endDayjsValueFormatFromNow = endDayjsValue.fromNow()
