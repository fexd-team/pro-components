...
describe('type="multipleSelect" 编辑模式', () => {
  // 能够正确渲染
  it('能够正确渲染', async () => {
    const { container } = render(<ProField className="test-field" type="multipleSelect" label="Test Label" />)
  ...
  it('初始值能正确赋值', async () => {
    const { container } = render(
      <ProField className="test-field" type="multipleSelect" initialValue={[2, 3]} options={mockOptions} />,
    )
  ...
  it('可以搜索选项', async () => {
    ...
    fireEvent.change(container.querySelector('.ant-select-selector .ant-select-selection-search-input')!, {
      target: { value: '选项1' },
    })
    await waitFor(() => {
      expect(screen.getByText('选项1', { selector: '.ant-select-item-option-content' })).toBeInTheDocument()
      expect(screen.queryByText('选项2', { selector: '.ant-select-item-option-content' })).not.toBeInTheDocument()
      expect(screen.queryByText('选项3', { selector: '.ant-select-item-option-content' })).not.toBeInTheDocument()
      expect(screen.queryByText('选项4', { selector: '.ant-select-item-option-content' })).not.toBeInTheDocument()
      expect(screen.queryByText('选项5', { selector: '.ant-select-item-option-content' })).not.toBeInTheDocument()
    })
  })

  // 能够通过上下文配置正确的语言
  it('能够通过上下文配置正确的语言', async () => {
    const { container: container1 } = render(<ProField className="test-field1" type="multipleSelect" />)
    ...
    const { container: container2 } = render(
      <ConfigProvider localeKey="zh_CN">
        <ProField className="test-field2" type="multipleSelect" />
      </ConfigProvider>,
    )
    await delay(100)
...
describe('type="multipleSelect" 浏览模式', () => {
  // 能够正确渲染
  it('能够正确渲染', async () => {
    const node = render(<ProField mode="view" type="multipleSelect" label="Test Label" />)
  ...
  // 初始值能正确显示
  it('初始值能正确显示', async () => {
    const node = render(<ProField mode="view" type="multipleSelect" initialValue={[2, 3]} options={mockOptions} />)
    expect(node.getByText('选项2')).toBeInTheDocument()
    expect(node.getByText('选项3')).toBeInTheDocument()
  })

  // 初始值可以类型模糊
  it('初始值能正确显示', async () => {
    const node = render(<ProField mode="view" type="multipleSelect" initialValue={['1', '4']} options={mockOptions} />)
    expect(node.getByText('选项1')).toBeInTheDocument()
    expect(node.getByText('选项4')).toBeInTheDocument()
  })
})
